...
function openApp(names) {
  ...
  return new Promise((resolve, reject) => {
    if (platform === "darwin") {
      childProcess.execFile("open", ["-a", candidates[0]], (error) => (error ? reject(error) : resolve()));
      return;
    }
...
const server = http.createServer(async (request, response) => {
  ...
  try {
    ...
    if (url.pathname === "/api/ps5/remote-play") {
      await openApp(process.platform === "darwin" ? "PS Remote Play" : ["PS Remote Play", "RemotePlay.exe"]);
      sendJson(response, 200, { ok: true, message: "PS Remote Play öppnad." });
      return;
    }

    if (url.pathname === "/api/samsung/key") {
      const result = await sendSamsungKey(body);
